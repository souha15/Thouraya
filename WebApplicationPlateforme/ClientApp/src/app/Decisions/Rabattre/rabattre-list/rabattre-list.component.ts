...
@Component({
  selector: 'app-rabattre-list',
  templateUrl: './rabattre-list.component.html',
  styleUrls: ['./rabattre-list.component.css']
})
export class RabattreListComponent implements OnInit {
  ...
  getList() {
    this.decService.List().subscribe(res => {
      this.DecList1 = res
      this.DecList = this.DecList1.filter(item => item.typeDecint == 1)
    })
  }
