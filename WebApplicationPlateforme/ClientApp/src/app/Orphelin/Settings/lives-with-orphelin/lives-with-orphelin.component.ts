...
@Component({
  selector: 'app-lives-with-orphelin',
  templateUrl: './lives-with-orphelin.component.html',
  styleUrls: ['./lives-with-orphelin.component.css']
})
export class LivesWithOrphelinComponent implements OnInit {
  ...
  ShowDotations() {
    this.allDotation = this.tblService.GetLivesWiths();
  }

  //Delete Dotation
  onDelete(Id) {
    if (confirm('Are you sure to delete this record ?')) {
      this.tblService.DeleteLivesWiths(Id)
        .subscribe(res => {
          this.ShowDotations();
          this.resetForm();
  ...
  updateRecord(form: NgForm) {
    this.tblService.EditLivesWiths().subscribe(
      res => {
        this.resetForm(form);
  ...
  insertRecord(form: NgForm) {
    this.tblService.PostLivesWiths().subscribe(
      res => {
        this.resetForm(form);
