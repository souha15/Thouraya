...
@Component({
  selector: 'app-parente-orphelin',
  templateUrl: './parente-orphelin.component.html',
  styleUrls: ['./parente-orphelin.component.css']
})
export class ParenteOrphelinComponent implements OnInit {
  ...
  ShowDotations() {
    this.allDotation = this.tblService.GetParente();
  }

  //Delete Dotation
  onDelete(Id) {
    if (confirm('Are you sure to delete this record ?')) {
      this.tblService.DeleteParente(Id)
        .subscribe(res => {
          this.ShowDotations();
          this.resetForm();
  ...
  updateRecord(form: NgForm) {
    this.tblService.EditParente().subscribe(
      res => {
        this.resetForm(form);
  ...
  insertRecord(form: NgForm) {
    this.tblService.PostParente().subscribe(
      res => {
        this.resetForm(form);
