...
@Component({
  selector: 'app-maladie-orphelin',
  templateUrl: './maladie-orphelin.component.html',
  styleUrls: ['./maladie-orphelin.component.css']
})
export class MaladieOrphelinComponent implements OnInit {
  ...
  ShowDotations() {
    this.allDotation = this.tblService.GetMaladies();
  }

  //Delete Dotation
  onDelete(Id) {
    if (confirm('Are you sure to delete this record ?')) {
      this.tblService.DeleteMaladies(Id)
        .subscribe(res => {
          this.ShowDotations();
          this.resetForm();
  ...
  updateRecord(form: NgForm) {
    this.tblService.EditMaladies().subscribe(
      res => {
        this.resetForm(form);
  ...
  insertRecord(form: NgForm) {
    this.tblService.PostMaladies().subscribe(
      res => {
        this.resetForm(form);
