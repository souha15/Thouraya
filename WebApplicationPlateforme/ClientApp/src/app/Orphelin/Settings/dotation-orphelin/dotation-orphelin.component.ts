...
@Component({
  selector: 'app-dotation-orphelin',
  templateUrl: './dotation-orphelin.component.html',
  styleUrls: ['./dotation-orphelin.component.css']
})
export class DotationOrphelinComponent implements OnInit {
  ...
  ShowDotations() {
    this.allDotation = this.tblService.GetTypeDotationOrphelins();
  }

  //Delete Dotation
  onDelete(Id) {
    if (confirm('Are you sure to delete this record ?')) {
      this.tblService.DeleteTypeDotationOrphelins(Id)
        .subscribe(res => {
          this.ShowDotations();
          this.resetForm();
  ...
  updateRecord(form: NgForm) {
    this.tblService.EditTypeDotationOrphelins().subscribe(
      res => {
        this.resetForm(form);
  ...
  insertRecord(form: NgForm) {
    this.tblService.PostTypeDotationOrphelins().subscribe(
      res => {
        this.resetForm(form);
