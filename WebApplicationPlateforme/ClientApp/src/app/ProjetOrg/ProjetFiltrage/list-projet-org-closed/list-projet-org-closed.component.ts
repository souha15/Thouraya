...
@Component({
  selector: 'app-list-projet-org-closed',
  templateUrl: './list-projet-org-closed.component.html',
  styleUrls: ['./list-projet-org-closed.component.css']
})
export class ListProjetOrgClosedComponent implements OnInit {
  ...
  getList() {
    this.projetService.Get().subscribe(res => {
      this.EvList2 = res
      this.EvList = this.EvList2.filter(item => item.etat == "مغلقة")
    })
  }
