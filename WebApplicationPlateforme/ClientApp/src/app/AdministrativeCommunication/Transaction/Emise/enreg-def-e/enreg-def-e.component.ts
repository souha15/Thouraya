...
@Component({
  selector: 'app-enreg-def-e',
  templateUrl: './enreg-def-e.component.html',
  styleUrls: ['./enreg-def-e.component.css']
})
export class EnregDefEComponent implements OnInit {
  filter;
  ...
  TransactionList() {
    ...
    this.transactionService.ListE().subscribe(res => {
      this.Globallist = res
      this.FiltredList = this.Globallist.filter(item => item.attribut6 == "حفظ نهائي" && item.idUserCreator == this.UserIdConnected)
  ...
  onDeleteTr() {
    if (confirm('هل أنت متأكد من حذف هذا الملف ?')) {
      this.transactionService.DeleteE(this.tr.id)
        .subscribe(res => {
          this.TransactionList();
  ...
  getBack() {
    ...
    this.transactionService.PutObservableE(this.tr).subscribe(
      res => {
