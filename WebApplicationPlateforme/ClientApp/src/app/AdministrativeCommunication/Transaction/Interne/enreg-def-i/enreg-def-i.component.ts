...
@Component({
  selector: 'app-enreg-def-i',
  templateUrl: './enreg-def-i.component.html',
  styleUrls: ['./enreg-def-i.component.css']
})
export class EnregDefIComponent implements OnInit {
  filter;
  ...
  onDeleteTr() {
    if (confirm('هل أنت متأكد من حذف هذا الملف ?')) {
      this.transactionService.DeleteI(this.tr.id)
        .subscribe(res => {
          this.TransactionList();
  ...
  getBack() {
    ...
    this.transactionService.PutObservableI(this.tr).subscribe(
      res => {
