...
@Component({
  selector: 'app-activite-dawa-list',
  templateUrl: './activite-dawa-list.component.html',
  styleUrls: ['./activite-dawa-list.component.css']
})
export class ActiviteDawaListComponent implements OnInit {
  ...
  list2: Activite[] = [];
  getActiviteList() {
    this.activiteService.ListD().subscribe(res => {
      this.list = res;
    })
  }
  ...
  onSubmit(form: NgForm) {
    if (form.invalid) {
      ...
      this.activiteService.PutObservableED(this.ac).subscribe(
        res => {
          this.getActiviteList();
  ...
  onDelete(Id) {
    if (confirm('Are you sure to delete this record ?')) {
      ...
      this.activiteService.DeleteD(Id)
        .subscribe(res => {
          this.getActiviteList()
