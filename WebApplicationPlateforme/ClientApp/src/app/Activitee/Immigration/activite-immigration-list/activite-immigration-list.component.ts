...
@Component({
  selector: 'app-activite-immigration-list',
  templateUrl: './activite-immigration-list.component.html',
  styleUrls: ['./activite-immigration-list.component.css']
})
export class ActiviteImmigrationListComponent implements OnInit {
  ...
  list2: Activite[] = [];
  getActiviteList() {
    this.activiteService.ListI().subscribe(res => {
      this.list = res;
    })
  }
  ...
  getTypeActiviteList() {
    this.typeService.GetI().subscribe(res => {
      this.typeList = res;
    })
  }
  ...
  onDelete(Id) {
    if (confirm('Are you sure to delete this record ?')) {
      ...
      this.activiteService.DeleteI(Id)
        .subscribe(res => {
          this.getActiviteList()
