...
@Component({
  selector: 'app-activite-prepa-list',
  templateUrl: './activite-prepa-list.component.html',
  styleUrls: ['./activite-prepa-list.component.css']
})
export class ActivitePrepaListComponent implements OnInit {
  ...
  list2: Activite[] = [];
  getActiviteList() {
    this.activiteService.ListP().subscribe(res => {
      this.list = res;
    })
  }
  ...
  onDelete(Id) {
    if (confirm('Are you sure to delete this record ?')) {
      ...
      this.activiteService.DeleteP(Id)
        .subscribe(res => {
          this.getActiviteList()
