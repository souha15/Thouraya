...
@Component({
  selector: 'app-ticket2-list-user',
  templateUrl: './ticket2-list-user.component.html',
  styleUrls: ['./ticket2-list-user.component.css']
})
export class Ticket2ListUserComponent implements OnInit {
  ...
  getTicketLis() {
    this.ticketService.ListTicket2().subscribe(res => {
      this.TicList2 = res;
      this.TicList = this.TicList2.filter(item => item.iduser == this.UserIdConnected || item.idadmin == this.idadmin || item.idetab == this.idetab)
