...
@Component({
  selector: 'app-chequec-edit',
  templateUrl: './chequec-edit.component.html',
  styleUrls: ['./chequec-edit.component.css']
})
export class ChequecEditComponent implements OnInit {
  ...
  constructor(private chequeService: ChequeCService,
    ...
    private filescService: FilesChequeService,
  ...
  getIdUrl() {
    this.routeSub = this.route.params.subscribe(params => {
      ...
      this.chequeService.GetById(this.cptId).subscribe(res => {
        this.ch = res
  ...
  // Get Files
  Filescheque: FilesChequesC[] = [];
  FileschequeF: FilesChequesC[] = [];
  filestest: boolean = false;
  getFilesList() {
    this.filescService.Get().subscribe(res => {
      this.FileschequeF = res
      this.Filescheque = this.FileschequeF.filter(item => item.idCheque == this.cptId)
      if (this.Filescheque.length != 0) {
        this.filestest = true;
      }
    })
  }
  ...
  getArticle() {
    this.articleService.Get().subscribe(res => {
      this.arlisF = res
      this.arlis = this.arlisF.filter(item => item.idCheque == this.cptId)
  ...
  deleteF(Id) {
    this.filescService.Delete(Id).subscribe(res => {
      this.getFilesList();
    })
  }
  ...
  onSubmit(form: NgForm) {
    if (form.invalid) {
      ...
      this.chequeService.PutObservableE(this.ch).subscribe(
        res => {
