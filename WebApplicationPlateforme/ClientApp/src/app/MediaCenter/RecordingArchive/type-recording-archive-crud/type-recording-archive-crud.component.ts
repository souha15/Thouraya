...
@Component({
  selector: 'app-type-recording-archive-crud',
  templateUrl: './type-recording-archive-crud.component.html',
  styleUrls: ['./type-recording-archive-crud.component.css']
})
export class TypeRecordingArchiveCrudComponent implements OnInit {
  ...
  ShowDotations() {
    this.allDotation = this.tblService.GetRecording();
  }

  //Delete Dotation
  onDelete(Id) {
    if (confirm('Are you sure to delete this record ?')) {
      this.tblService.DeleteRecording(Id)
        .subscribe(res => {
          this.ShowDotations();
          this.resetForm();
  ...
  updateRecord(form: NgForm) {
    this.tblService.EditRecording().subscribe(
      res => {
        this.resetForm(form);
  ...
  insertRecord(form: NgForm) {
    this.tblService.PostRecording().subscribe(
      res => {
        this.resetForm(form);
