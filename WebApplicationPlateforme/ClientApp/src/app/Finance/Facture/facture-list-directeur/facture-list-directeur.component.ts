...
@Component({
  selector: 'app-facture-list-directeur',
  templateUrl: './facture-list-directeur.component.html',
  styleUrls: ['./facture-list-directeur.component.css']
})
export class FactureListDirecteurComponent implements OnInit {
  filter;
  ...
  getFactures() {
    this.factureService.Get().subscribe(res => {
      this.GfactList = res
      this.factList = this.GfactList.filter(item => item.attribut2 == "معتمدة")
    })
  }
  ...
  getBack() {
    this.fact.attribut2 = "جاهزة للصرف"
    this.fact.etatdirector = "جاهزة للصرف"
    this.fact.dateetatdirector = this.date;
    this.fact.directorid = this.UserIdConnected;
    this.fact.directornom = this.UserNameConnected;
