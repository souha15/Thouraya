...
@Component({
  selector: 'app-projet-prog',
  templateUrl: './projet-prog.component.html',
  styleUrls: ['./projet-prog.component.css']
})
export class ProjetProgComponent implements OnInit {
  ...
  ShowDotations() {
    this.allDotation = this.tblService.GetProjetProg();
  }

  //Delete Dotation
  onDelete(Id) {
    if (confirm('Are you sure to delete this record ?')) {
      this.tblService.DeleteProg(Id)
        .subscribe(res => {
          this.ShowDotations();
          this.resetForm();
  ...
  updateRecord(form: NgForm) {
    this.tblService.EditProg().subscribe(
      res => {
        this.resetForm(form);
  ...
  insertRecord(form: NgForm) {
    this.tblService.PostProg().subscribe(
      res => {
        this.resetForm(form);
