...
@Component({
  selector: 'app-projet-pay-type',
  templateUrl: './projet-pay-type.component.html',
  styleUrls: ['./projet-pay-type.component.css']
})
export class ProjetPayTypeComponent implements OnInit {
  ...
  ShowDotations() {
    this.allDotation = this.tblService.GetPayType();
  }

  //Delete Dotation
  onDelete(Id) {
    if (confirm('Are you sure to delete this record ?')) {
      this.tblService.DeletePayType(Id)
        .subscribe(res => {
          this.ShowDotations();
          this.resetForm();
  ...
  updateRecord(form: NgForm) {
    this.tblService.EditPayType().subscribe(
      res => {
        this.resetForm(form);
  ...
  insertRecord(form: NgForm) {
    this.tblService.PostPayType().subscribe(
      res => {
        this.resetForm(form);
