...
@Component({
  selector: 'app-projet-class',
  templateUrl: './projet-class.component.html',
  styleUrls: ['./projet-class.component.css']
})
export class ProjetClassComponent implements OnInit {
  ...
  ShowDotations() {
    this.allDotation = this.tblService.GetClass();
  }

  //Delete Dotation
  onDelete(Id) {
    if (confirm('Are you sure to delete this record ?')) {
      this.tblService.DeleteClass(Id)
        .subscribe(res => {
          this.ShowDotations();
          this.resetForm();
  ...
  updateRecord(form: NgForm) {
    this.tblService.EditClass().subscribe(
      res => {
        this.resetForm(form);
  ...
  insertRecord(form: NgForm) {
    this.tblService.PostClass().subscribe(
      res => {
        this.resetForm(form);
