...
@Component({
  selector: 'app-tache-ev-c',
  templateUrl: './tache-ev-c.component.html',
  styleUrls: ['./tache-ev-c.component.css']
})
export class TacheEvCComponent implements OnInit {
  ...
  ShowDotations() {
    this.allDotation = this.tblService.GetTache();
  }

  //Delete Dotation
  onDelete(Id) {
    if (confirm('Are you sure to delete this record ?')) {
      this.tblService.DeleteT(Id)
        .subscribe(res => {
          this.ShowDotations();
          this.resetForm();
  ...
  updateRecord(form: NgForm) {
    this.tblService.EditT().subscribe(
      res => {
        this.resetForm(form);
  ...
  insertRecord(form: NgForm) {
    this.tblService.PostT().subscribe(
      res => {
        this.resetForm(form);
