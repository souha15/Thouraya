...
@Component({
  selector: 'app-outils-c',
  templateUrl: './outils-c.component.html',
  styleUrls: ['./outils-c.component.css']
})
export class OutilsCComponent implements OnInit {
  ...
  ShowDotations() {
    this.allDotation = this.tblService.GetOutils();
  }

  //Delete Dotation
  onDelete(Id) {
    if (confirm('Are you sure to delete this record ?')) {
      this.tblService.DeleteO(Id)
        .subscribe(res => {
          this.ShowDotations();
          this.resetForm();
  ...
  updateRecord(form: NgForm) {
    this.tblService.EditO().subscribe(
      res => {
        this.resetForm(form);
  ...
  insertRecord(form: NgForm) {
    this.tblService.PostO().subscribe(
      res => {
        this.resetForm(form);
