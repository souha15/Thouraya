...
@Component({
  selector: 'app-beneficiaire-c',
  templateUrl: './beneficiaire-c.component.html',
  styleUrls: ['./beneficiaire-c.component.css']
})
export class BeneficiaireCComponent implements OnInit {
  ...
  ShowDotations() {
    this.allDotation = this.tblService.GetBeneficiaire();
  }

  //Delete Dotation
  onDelete(Id) {
    if (confirm('Are you sure to delete this record ?')) {
      this.tblService.DeleteB(Id)
        .subscribe(res => {
          this.ShowDotations();
          this.resetForm();
  ...
  updateRecord(form: NgForm) {
    this.tblService.EditB().subscribe(
      res => {
        this.resetForm(form);
  ...
  insertRecord(form: NgForm) {
    this.tblService.PostB().subscribe(
      res => {
        this.resetForm(form);
