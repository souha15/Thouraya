...
@Component({
  selector: 'app-langue-ev',
  templateUrl: './langue-ev.component.html',
  styleUrls: ['./langue-ev.component.css']
})
export class LangueEvComponent implements OnInit {
  ...
  ShowDotations() {
    this.allDotation = this.tblService.GetLangue();
  }

  //Delete Dotation
  onDelete(Id) {
    if (confirm('Are you sure to delete this record ?')) {
      this.tblService.DeleteLangue(Id)
        .subscribe(res => {
          this.ShowDotations();
          this.resetForm();
  ...
  updateRecord(form: NgForm) {
    this.tblService.EditLangue().subscribe(
      res => {
        this.resetForm(form);
  ...
  insertRecord(form: NgForm) {
    this.tblService.PostLangue().subscribe(
      res => {
        this.resetForm(form);
