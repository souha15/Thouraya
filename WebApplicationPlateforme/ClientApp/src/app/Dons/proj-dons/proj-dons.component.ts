...
@Component({
  selector: 'app-proj-dons',
  templateUrl: './proj-dons.component.html',
  styleUrls: ['./proj-dons.component.css']
})
export class ProjDonsComponent implements OnInit {
  ...
  ShowDotations() {
    this.allDotation = this.tblService.GetProjetDons();
  }

  //Delete Dotation
  onDelete(Id) {
    if (confirm('Are you sure to delete this record ?')) {
      this.tblService.DeletePD(Id)
        .subscribe(res => {
          this.ShowDotations();
          this.resetForm();
  ...
  updateRecord(form: NgForm) {
    this.tblService.EditPD().subscribe(
      res => {
        this.resetForm(form);
  ...
  insertRecord(form: NgForm) {
    this.tblService.PostPD().subscribe(
      res => {
        this.resetForm(form);
