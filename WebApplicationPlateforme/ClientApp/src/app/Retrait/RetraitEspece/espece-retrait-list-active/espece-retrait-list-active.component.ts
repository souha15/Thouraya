...
@Component({
  selector: 'app-espece-retrait-list-active',
  templateUrl: './espece-retrait-list-active.component.html',
  styleUrls: ['./espece-retrait-list-active.component.css']
})
export class EspeceRetraitListActiveComponent implements OnInit {
  ...
  getList() {
    this.retraitService.List().subscribe(res => {
      this.retList1 = res;
      this.retList = this.retList1.filter(item => item.typeRetrait == "يدوي" && item.etat == "مفعل");
    })
  }
