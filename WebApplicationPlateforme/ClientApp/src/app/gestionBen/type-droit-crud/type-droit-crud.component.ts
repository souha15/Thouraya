...
@Component({
  selector: 'app-type-droit-crud',
  templateUrl: './type-droit-crud.component.html',
  styleUrls: ['./type-droit-crud.component.css']
})
export class TypeDroitCrudComponent implements OnInit {
  ...
  ShowDotations() {
    this.allDotation = this.tblService.GetTD();
  }

  //Delete Dotation
  onDelete(Id) {
    if (confirm('Are you sure to delete this record ?')) {
      this.tblService.DeleteTD(Id)
        .subscribe(res => {
          this.ShowDotations();
          this.resetForm();
  ...
  updateRecord(form: NgForm) {
    this.tblService.EditTD().subscribe(
      res => {
        this.succ = true;
        this.failed = false;
  ...
  insertRecord(form: NgForm) {
    this.tblService.PostTD().subscribe(
      res => {
        this.succ = true;
        this.failed = false;
