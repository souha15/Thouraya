...
@Component({
  selector: 'app-type-rev-ben-crud',
  templateUrl: './type-rev-ben-crud.component.html',
  styleUrls: ['./type-rev-ben-crud.component.css']
})
export class TypeRevBenCrudComponent implements OnInit {
  ...
  ShowDotations() {
    this.allDotation = this.tblService.GetTR();
  }

  //Delete Dotation
  onDelete(Id) {
    if (confirm('Are you sure to delete this record ?')) {
      this.tblService.DeleteTR(Id)
        .subscribe(res => {
          this.ShowDotations();
          this.resetForm();
  ...
  updateRecord(form: NgForm) {
    this.tblService.EditTR().subscribe(
      res => {
        this.resetForm(form);
  ...
  insertRecord(form: NgForm) {
    this.tblService.PostTR().subscribe(
      res => {
        this.resetForm(form);
